...
Розширена версія з фільтрами: інтереси, час, максимальна відстань. Також повертає дані хоста.

### get_my_chats_optimized(p_user_id)
Оптимізований запит для списку чатів. Повертає все одним запитом: чат, учасники, останнє повідомлення, unread count. Фільтрує порожні чати завершених live-подій.
