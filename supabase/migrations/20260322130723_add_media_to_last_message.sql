-- Add image_url and audio_url to get_my_chats_optimized function
-- This allows the chat list to display media message types correctly
-- Also fixes column names: profiles has first_name/last_name, not username

-- Drop old function first
DROP FUNCTION IF EXISTS public.get_my_chats_optimized(UUID);

-- Recreate with media fields and correct column names
...
BEGIN
  ...
  SELECT 
    ...
    -- Last message details (including media URLs)
  ...
  -- Join to get last message (including media URLs)
  LEFT JOIN LATERAL (
    SELECT msg.content, msg.created_at, msg.sender_id, msg.read, msg.image_url, msg.audio_url
    FROM public.messages msg
    WHERE msg.chat_id = c.id
  ...
  GROUP BY c.id, c.type, c.walk_id, c.updated_at, w.title, w.image_url, w.start_time,
           m.content, m.created_at, m.sender_id, m.read, m.image_url, m.audio_url
  ORDER BY c.updated_at DESC;
END;
$$;

COMMENT ON FUNCTION public.get_my_chats_optimized IS 
  'Optimized chat loading function with media support. Returns all chat data including last message media URLs in single query.';
