...
LocalMeet використовує гібридну систему чатів, що підтримує як групові чати для івентів, так і особисті повідомлення.

### Групові чати
- **Автоматичне створення**: Кожен івент автоматично отримує груповий чат
- **Управління учасниками**: Організатор івенту стає власником чату
- **Додавання учасників**: Люди автоматично додаються при прийнятті запиту на участь
- **Передача власності**: Якщо власник покидає чат, права передаються іншому учаснику

### Особисті чати
- **Мігровані з попередньої системи**: Всі існуючі 1-на-1 чати збережені
- **Рівні права**: Обидва учасники мають однакові можливості
- **Збереження історії**: Вся історія повідомлень залишається доступною

### Можливості чатів
- **Типи повідомлень**: Текст, зображення, аудіо
- **Реальний час**: Миттєва доставка повідомлень всім учасникам
- **Статус прочитання**: Відстеження непрочитаних повідомлень
- **Профілі відправників**: Кожне повідомлення містить інформацію про відправника

### Приклади використання

```typescript
// Отримати всі чати користувача (групові та особисті)
const chats = await getMyChats(userId);

// Отримати повідомлення для будь-якого чату
const messages = await getChatMessages(chatId);

// Відправити повідомлення (працює для обох типів чатів)
await sendMessage(chatId, userId, {
  content: 'Привіт всім!',
  type: 'text'
});

// Управління груповими чатами
await leaveChat(chatId, userId);
await removeChatParticipant(chatId, participantId); // Тільки власник
...
### Філософія дизайну
Мінімалістичний, чистий, сучасний дизайн з фокусом на контент та зручність використання. Без зайвих декорацій, іконки на кнопках тільки коли необхідно, консистентні відступи та типографіка.

### Кольорова палітра

#### Основні кольори
- **ACCENT_ORANGE** (#FF9500) - Основні дії, активні стани, акценти
- **TEXT_DARK** (#333333) - Основний текст, заголовки
- **TEXT_LIGHT** (#999999) - Вторинний текст, лейбли, placeholder

#### Фонові кольори
- **BG_SECONDARY** (#F2F2F7) - Фон екранів
- **CARD_BG** (#FFFFFF) - Картки, інпути, піднесені поверхні
- **INPUT_BG** (#F2F2F7) - Альтернативний фон інпутів

#### Утилітарні кольори
- **BORDER_COLOR** (#E8E8E8) - Роздільники, бордери
- **SUCCESS_GREEN** (#4CAF50) - Успішні стани
- **ERROR_RED** (#FF3B30) - Помилки, деструктивні дії

### Типографіка

#### Розміри шрифтів
- **34px** - Великі заголовки (екрани авторизації)
- **28px** - Заголовки сторінок
- **22-24px** - Заголовки секцій, модальних вікон
- **17-18px** - Текст кнопок, важливі лейбли
- **16px** - Основний текст, текст інпутів
- **14-15px** - Вторинний текст, описи
- **11-12px** - Uppercase лейбли, підказки

#### Вага шрифту
- **700** - Заголовки
- **600** - Лейбли, кнопки, акценти
- **500** - Звичайний текст

#### Стилі тексту
- **Uppercase лейбли**: 11-12px, fontWeight 600, letterSpacing 0.5
- **Заголовки**: 22-34px, fontWeight 700
- **Основний текст**: 15-16px, fontWeight 400-500
- **Кнопки**: 16-17px, fontWeight 600

### Відступи

#### Padding
- **Екрани**: 20px горизонтально
- **Картки**: 16-20px
- **Інпути**: 14-16px вертикально, 16px горизонтально
- **Кнопки**: 16-18px вертикально
- **Між секціями**: 16-20px (використовуй gap)

#### Margins
- **Між секціями**: 20-24px
- **Між елементами**: 8-12px
- **Під заголовком**: 40px
- **Safe area зверху**: insets.top + 60px
...
- **Кнопки**: 16-20px
- **Чіпси**: 20-24px
- **Модальні вікна**: 24px (верхні кути)

### Тіні

#### Стандартна тінь (картки, інпути)
```typescript
shadowColor: '#000',
...
**ВАЖЛИВО**: Ніколи не використовуй кольорові тіні (наприклад, shadowColor: COLORS.ACCENT_ORANGE). Завжди використовуй чорний колір з низькою прозорістю.

### Компоненти

#### Кнопки
- **Основні**: Помаранчевий фон, білий текст, 16px padding, 16px border radius
- **Без іконок**: Кнопки не повинні мати іконок, якщо це не критично необхідно
- **Disabled стан**: opacity 0.6

#### Інпути
- **Фон**: Білий (CARD_BG) з тінню
- **Без бордерів**: Використовуй тіні замість бордерів
- **Padding**: 14px вертикально, 16px горизонтально
- **Border radius**: 12px
- **Лейбл**: Uppercase, 14px, над інпутом з відступом 8px

#### Картки
- **Фон**: Білий (CARD_BG)
- **Тінь**: Стандартна тінь (0.06 opacity)
- **Border radius**: 16-20px
- **Padding**: 16-20px
- **На сірому фоні**: Завжди використовуй BG_SECONDARY для фону екранів

#### Лейбли
- **Стиль**: Uppercase, 11-14px, fontWeight 600, letterSpacing 0.5
- **Колір**: TEXT_DARK або TEXT_LIGHT
- **Позиція**: Над інпутами/секціями з відступом 8-12px

#### Bottom Sheets / Модальні вікна
- **Фон**: BG_SECONDARY (сірий)
- **Border radius**: 24px (тільки верхні кути)
- **Handle**: 40x4px, BORDER_COLOR, по центру, 12-20px margin bottom
- **Padding**: 20px горизонтально, 40px знизу
- **Без кнопок закриття**: Використовуй жест свайпу вниз (якщо не критично)
...
- **Padding**: 12px
- **Border radius**: 12px
- **Розмір шрифту**: 14px, fontWeight 500
...
<View style={{ backgroundColor: COLORS.BG_SECONDARY }}>
  <View style={{ padding: 20, paddingTop: insets.top + 60 }}>
    <View style={styles.header}>
      ...
      <Text style={styles.subtitle}>Підзаголовок</Text>
    </View>
    
    <View style={styles.content}>
      {/* Картки з білим фоном та тінями */}
    </View>
  </View>
</View>
```
...
<View style={{ gap: 20 }}>
  <View style={{ gap: 8 }}>
    <Text style={styles.label}>ЛЕЙБЛ</Text>
    <TextInput style={styles.input} />
  </View>
  
  <TouchableOpacity style={styles.button}>
    <Text style={styles.buttonText}>Дія</Text>
  </TouchableOpacity>
</View>
```
...
#### РОБИТИ
- Використовуй COLORS константи з `@shared/constants`
- Використовуй gap для відступів між елементами
- Додавай тіні до карток та інпутів (0.06 opacity)
- Використовуй uppercase лейбли з letterSpacing
- Тримай padding консистентним (20px екрани, 16-20px картки)
- Використовуй сірі фони (BG_SECONDARY) для екранів
- Використовуй білі фони (CARD_BG) для карток/інпутів
- Групуй пов'язані інпути з лейблами (inputGroup pattern)

#### НЕ РОБИТИ
- Не використовуй бордери на інпутах (використовуй тіні)
- Не використовуй кольорові тіні на кнопках
- Не додавай іконки до кнопок без необхідності
- Не використовуй hardcoded кольори (використовуй COLORS)
- Не використовуй marginBottom (використовуй gap)
- Не додавай кнопки закриття до модальних вікон (використовуй жест)
- Не використовуй білі фони для екранів
- Не використовуй важкі тіні (тримай opacity низьким)

### Детальна документація
Повна документація дизайн-системи доступна в `.amazonq/rules/design-system.md`
